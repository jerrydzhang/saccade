...
async def main() -> None:
    ...
    memory.set_system("You are a helpful assistant. Use tools when appropriate. Be concise.")

    print("\n" + "=" * 60)
    print("SCENARIO 3: Parallel Tool Calls")
    print("=" * 60)

    with Trace() as trace:
        memory.add_user("Calculate 15 + 27 and also 8 * 9")
        print("\n[user] Calculate 15 + 27 and also 8 * 9")
        ...
        if response1.tool_calls:
            print(f"[assistant] Requested {len(response1.tool_calls)} tool calls:")
