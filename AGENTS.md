...
- One decision at a time, in conversation. The human ratifies; then it lands
  in `DESIGN.md`. Arguments are welcome — when a counter-argument lands,
...
- Do not import conversational coinage into code or docs. A term earns its
  place by being ratified, and check for collisions with existing non-goals
  (`DESIGN.md` promises "no learn semantics anywhere" — don't add one).
...
- `tests/mapping_corpus.rs`: worked examples of the beads mapping (§12).
